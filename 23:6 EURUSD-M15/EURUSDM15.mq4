...
void EvaluateEntry(){
   ...
   //TO-DO: get the PVA Candles -> Compare with the Trend and Dragon
   ...
   if(TotalOpenBuy <= 0 && upObv == true && vol == true && trend < dragonC && Close[1] > Open[1] && Open[0] > dragonH && iADX(Symbol(), PERIOD_CURRENT, 20, PRICE_CLOSE, MODE_PLUSDI, 0) > iADX(Symbol(), PERIOD_CURRENT, 20, PRICE_CLOSE, MODE_MINUSDI, 0)){
  ...
  if( TotalOpenSell <= 0 &&downObv == true && vol == true && trend > dragonC && Close[1] < Open[1] && Open[0] < dragonL && iADX(Symbol(), PERIOD_CURRENT, 20, PRICE_CLOSE, MODE_PLUSDI, 0) < iADX(Symbol(), PERIOD_CURRENT, 20, PRICE_CLOSE, MODE_MINUSDI, 0)){ 
...
void EvaluateExit(){
   ...
   if(OrderType() == OP_BUY && (iClose(Symbol(), PERIOD_CURRENT, 0) >= 1.0025*OrderOpenPrice() || TimeCurrent()-OrderOpenTime() > 14400)){
   ...
   if(OrderType() == OP_SELL && (iClose(Symbol(), PERIOD_CURRENT, 0) <= 0.9975 * OrderOpenPrice()  || TimeCurrent()-OrderOpenTime() > 14400)){
...
bool increasingVol(){
   ...
   double currVol = iVolume(Symbol(), PERIOD_CURRENT, 0);
  for(int i=1; i <= 10; i++){
   averageP10Vol += iVolume(Symbol(),PERIOD_CURRENT,  i)/10;
}
